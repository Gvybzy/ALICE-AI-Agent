...
AGENT_FOLDER = r"C:\Users\{}\OneDrive\Desktop\VS Code\projects\Ideas\ALICE_V1".format(USERNAME)
...
def chat_with_ai(prompt):
    """Talk to Groq AI - free, fast."""
    try:
        url = "https://api.groq.com/openai/v1/chat/completions"
        data = json.dumps({
            "model": "llama-3.1-8b-instant",
            "messages": [
                {"role": "system", "content": "You are ALICE, a friendly AI assistant. Keep responses concise and warm."},
...
def print_home_screen(greeting):
    ...
    print(center("=" * 40))
    print(center("Type a command, 'chat', 'voice', or 'help'"))
    print(center("─" * 30))
    print()
...
def open_app(app_name):
    ...
    speak(f"Sorry, I don't know {app_name}")
    separator()
...
def try_open(command):
    try:
        if command.startswith("start "):
            ...
            os.system(f"start {command} >nul 2>&1")
...
def launch(args):
    try:
        subprocess.Popen(args, shell=False, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, creationflags=subprocess.CREATE_NO_WINDOW)
        return True
    except Exception as e:
        print(center(f"Couldn't open that: {e}"))
        return False
...
def main():
    ...
    speak(f"{greeting} Doctor. What should we do today?")
    ...
    while True:
        ...
        os.system("taskkill /f /im powershell.exe 2>nul >nul")

        if command in ["exit", "close sys"]:
        ...
        if command == "chat":
            print(center("=" * 40))
            print(center("AI CHAT MODE"))
            print(center("Type 'stop' to exit"))
            print(center("=" * 40))
            speak("Chat mode activated")
            while True:
                chat_input = input("                                                         ").strip()
                if chat_input.lower() in ["stop", "exit", "quit"]:
                    os.system("taskkill /f /im powershell.exe 2>nul >nul")
                    print(center("Exited chat mode."))
                    speak("Exiting chat mode")
                    break
                if chat_input.lower() == "wait":
                    os.system("taskkill /f /im powershell.exe 2>nul >nul")
                    print(center("Voice stopped."))
                    continue
                if not chat_input:
                    continue
                print(center("Thinking..."))
                reply = chat_with_ai(chat_input)
                print()
        ...
        if command == "school":
            ...
            os.system("start https://uepcollege.acctech.ph")
            os.system("start https://ueplms.orangeapps.ph/oa_school")
            print(center("School mode activated"))
            speak("School mode activated")
            separator()
            continue

        if command == "study":
            print(center("Starting study mode..."))
            launch([r"C:\Users\{}\OneDrive\Desktop\VS Code\Code.exe".format(USERNAME)])
            launch([r"C:\Program Files\BraveSoftware\Brave-Browser\Application\brave.exe"])
        ...
        if command == "gaming":
            ...
            launch([r"C:\Program Files (x86)\Steam\Steam.exe"])
        ...
        if command == "chill":
            ...
            os.system(r'"C:\Program Files (x86)\Microsoft\Edge\Application\msedge_proxy.exe" --profile-directory=Default --app-id=kippjfofjhjlffjecoapiogbkgbpmgej --app-url=https://www.facebook.com/?ref=homescreenpwa --app-launch-source=4')
            os.system("start https://youtube.com")
        ...
        if command.startswith("folder "):
            ...
            if found:
                ...
                speak(f"Couldn't find folder {keyword}")
            separator()
            continue
